...
echo "";
(apt update -y && apt upgrade ) &> /dev/null;
apt install figlet pv ncurses-utils binutils coreutils wget git zsh termux-api procps gawk exa termux-tools -y &> /dev/null;
rubygem_d &> /dev/null
termux-wake-lock;
if [ -e $PREFIX/share/figlet/navajo11.flf ]; then
	echo -e "\e[1;34m[*] \033[32mnavajo.flf figlet font is present\033[0m";
	sleep 4
else
wget https://raw.githubusercontent.com/navajo11/navajo11/master/navajo11.flf &> /dev/null;
sleep 3
cp navajo11.flf $PREFIX/share/figlet/navajo11.flf;
...
clear;
echo -e "\033[31m$(figlet -f navajo "T- Header")\e[0m"
echo -e "\e[1;32m
+----------------------------------*/
...
while [ $ozsh = 0 ];
do
	echo -e "\e[1;34m[*] \e[32mna-va-jo new setup....\e[0m";
	echo "";

	( rm -rf ~/.zshrc;git clonehttps://github.com/navajo21/navajo11.git $HOME/.na-va-jo;cp "$HOME/.oh-my-zsh/templates/zshrc.zsh-template" "$HOME/.zshrc";termux-wake-unlock; ) &> /dev/null & spin;
	chsh -s zsh;
if [ -d $HOME/.na-va-jo ];
then
	ozsh=1
...
while [ $zshau = 0 ];
do
	( git clone --depth 1https://github.com/navajo21/navajo11.git~/.plugins/zsh-autosuggestions; echo "source ~/.plugins/zsh-autosuggestions/zsh-autosuggestions.zsh" >> $HOME/.zshrc; ) &> /dev/null & spin
